...
def move_toward(Rover,dist,ang,maxSpeed,followWall):
    if len(ang) >= Rover.stop_forward:  
        ...
        Roversteer = offset + np.clip(np.mean(ang * 180 / np.pi), -15,15)
...
def turn(Rover,ang):
    ...
    if round(Rover.yaw) == round(ang):
        ...
        if Rover.yaw > ang:
            sign = -1
        Roversteer = np.clip((abs(Rover.yaw) - abs(ang))* sign,-15,15)
        if Rover.steer>Roversteer:
            Rover.steer -=1
        else:
            Rover.steer +=1
...
#        Rover = move_toward(Rover,Rover.rock_dists,Rover.rock_angles,Rover.max_vel/2,False)
        ...
        if Rover.mode == 'forward': 
            ...
            if Rover.avgspd < 1:  #we are stuck or starting
                if (Rover.avgspd==0):	# starting
                    print('starting, let''s periot ' )
                    Rover.stopAngle = Rover.yaw + 359
                else:
                    print('stuck, average speed is ' + str(Rover.avgspd))
                    Rover.stopAngle = Rover.yaw + -180
                Rover.avgspd = nSamples * 10 # give it a chance by setting the average very wrong 
                Rover.mode = 'turning'
            else:
